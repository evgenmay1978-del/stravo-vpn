...
class VpnBootReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val allowed = if (intent.action == Intent.ACTION_BOOT_COMPLETED) settings.startOnBoot else settings.autoReconnect
        if (!allowed || !TunnelSession(context).wanted || VpnService.prepare(context) != null) return
        runCatching {
